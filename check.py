...
def check(king, queen):
    ...
    if king[1] == queen[1]: # if number coordinates are =
        return True
    elif king[0] == queen[0]: # if letter coordinates are =
        return True
    elif abs(dict_letters[king[0]] - dict_letters[queen[0]]) == abs(int(king[1]) - int(queen[1])):
        return True
    return False
