...
class Node(object):
    ...
    def insert(self, new_data):
        ...
        # traverse through tree by comparing value of new_data to self.data
        # if greater, go right
        # if less than, go left
        # once hit a leaf (no more children), append node somehow
        # if not self.left and not self.right:
        #     return self.left = Node(new_data)
        # if not self.right:
        #     return self.right = Node(new_data)
            # node.
        # else:
        if self.data <= new_data:
            if not self.right:
                ...
                insert(self.right, new_data)
        else: # if self.data >= new_data
            if not self.left:
                self.left = Node(new_data)
            else:
                insert(self.left, new_data)
